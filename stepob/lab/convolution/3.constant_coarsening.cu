...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < FILTER_SIZE * FILTER_SIZE; i++) filter[i] = static_cast<filter_type>(rand()) / RAND_MAX;
  ...
  //  Call CONSTANT MEM COARSENING GPU convolution kernel
  // Same (block, grid) as above
  const dim3 threadsPerBlock(IN_TILE_DIM, IN_TILE_DIM);
  const dim3 numBlocks(GRID_DIM,GRID_DIM);
  CHECK(cudaMemcpyToSymbol(constant_filter, filter, FILTER_SIZE * FILTER_SIZE * sizeof(filter_type)));
  convolution_constant_mem_coarsening_kernel<<<numBlocks, threadsPerBlock>>>(d_input, d_output, width, height);
  CHECK_KERNELCALL();
  ...
  for (int i = 0; i < width * height; i++) {
    if (std::abs(output_cpu[i] - output_gpu[i]) > 1e-3) {
      std::cout << "Convolution CONSTANT MEMORY COARSENING results are NOT equivalent!" << std::endl;
      return EXIT_FAILURE;
    }
  }

  std::cout << "Convolution CONSTANT MEMORY COARSENING results are equivalent!" << std::endl;
