...
__device__ int
  ...
  while (active) {
    if (i > 0 && j < n && A[i - 1] > B[j]) {
      // The same results as ceil(i - i_low, 2)
...
__global__ void merge_gpu(const int *__restrict__ A,
  ...
  const int k_curr = tid * gpu_ceil(dim_C, blockDim.x * gridDim.x);
  const int k_next = min((tid + 1) * gpu_ceil(dim_C, blockDim.x * gridDim.x), dim_C);

  const int i_curr = co_rank(k_curr, A, dim_A, B, dim_B);
  const int i_next = co_rank(k_next, A, dim_A, B, dim_B);

  const int j_curr = k_curr - i_curr;
  const int j_next = k_next - i_next;
