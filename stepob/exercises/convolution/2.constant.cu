...
__global__ void kernel(const int *input, const int *filter, int *res)
{
    ...
    for(int row=0; row<FILTER_DIM; row++){
        for(int col=0; col<FILTER_DIM; col++){
            ...
            int input_row = thread_row - FILTER_RADIUS + row;
            int input_column = thread_column - FILTER_RADIUS + col;
    ...
    res[thread_row*MATRIX_DIM + thread_column] = acc;
}
...
int main()
{
    ...
    dim3 blocksPerGrid((MATRIX_DIM + BLOCKDIM - 1) / BLOCKDIM, (MATRIX_DIM + BLOCKDIM - 1) / BLOCKDIM, 1);
