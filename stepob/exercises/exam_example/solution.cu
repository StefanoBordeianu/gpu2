...
//   for(int stride=0; stride<num; stride+=(BLOCKSIZE*GRIDSIZE)){
//     tile[DIST+threadIdx.x] = V[blockIdx.x*blockDim.x+threadIdx.x+stride];
//     if(threadIdx.x<DIST){
//       if(blockIdx.x>0)
//         tile[threadIdx.x] = V[blockIdx.x*blockDim.x+threadIdx.x-DIST+stride];
//       else
//         tile[threadIdx.x] = 0;
//     }
//     if(threadIdx.x>=blockDim.x-DIST){
//       if(blockIdx.x<gridDim.x-1)
//         tile[threadIdx.x+DIST*2] = V[(blockIdx.x)*blockDim.x+threadIdx.x+DIST+stride];
//       else
//         tile[DIST*2+threadIdx.x] = 0;  
//     }
//     __syncthreads();
...
int main(int argc, char **argv) {
  ...
  dim3 blocksPerGrid2(GRIDSIZE, 1, 1);
  dim3 threadsPerBlock2(BLOCKSIZE, 1, 1);
