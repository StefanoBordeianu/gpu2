...
__global__ void kernel(int *input, int *res, int dim)
{
    ...
    for(; index<dim; index+=(BLOCKDIM*GRIDSIZE)){
        tmp=1;
        shared_array[DIST+threadIdx.x] = input[index];
        if(threadIdx.x<DIST){
            if(index-DIST>0)
                shared_array[threadIdx.x] = input[index-DIST];
            else
                shared_array[threadIdx.x] = 0;
        }
        if(threadIdx.x>=blockDim.x-DIST){
            if(index-DIST<dim)
                shared_array[threadIdx.x+DIST*2] = input[index+DIST];
            else
        ...
        __syncthreads();
        if(index<dim){
            my_element = shared_array[threadIdx.x+DIST];
...
//     if (i < dim){
//         for(; i<dim; i+=(BLOCKDIM*GRIDSIZE)){
//             res[i] = 1;
//             my_element = input[i];

//             for(int j=1;j<=DIST;j++){
//                 if((i-j)>0 && (i-j)<dim){
//                     if(input[i-j]>=my_element)
//                         res[i] = 0;
//                 }
//             }
//             for(int j=1;j<=DIST;j++){
